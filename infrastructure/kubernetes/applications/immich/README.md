...
- OAuth/OIDC login means [Keycloak](../../security/rights-management/keycloak) accounts work here too
...
The phones keep using their native camera apps. The Immich app syncs the camera roll to the homelab in the background; photos are then viewable on every device through app or browser. External access for uploads on the go runs through [Cloudflare Tunnel](../../networking/cloudflare-tunnel).
...
| Dependency | Purpose |
|---|---|
| [`../../databases/postgresql`](../../databases/postgresql) | Metadata database (requires the pgvecto.rs/vector extension) |
| [`../../databases/redis`](../../databases/redis) | Job queue and caching |
| [`../../storage/longhorn`](../../storage/longhorn) | Photo/video storage volumes |
| [`../../networking/traefik`](../../networking/traefik) | HTTP(S) ingress |
| [`../../networking/cert-manager`](../../networking/cert-manager) | TLS certificates |
| [`../../networking/cloudflare-tunnel`](../../networking/cloudflare-tunnel) | Mobile upload from outside the LAN |
| [`../../security/rights-management/keycloak`](../../security/rights-management/keycloak) | Family SSO via OIDC |
| [`../../backup/velero`](../../backup/velero) | Backups — irreplaceable data lives here |
