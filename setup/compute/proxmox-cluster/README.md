...
- **Networking is the headline feature:** 2× **10G SFP+** and 2× **2.5G RJ45** onboard. Four ports are exactly enough to give every plane its own wire: SFP+ 1 is the tagged trunk into the MikroTik CRS310 (🔵 all VLANs, all homelab and storage traffic), 2.5G #1 terminates the internet as OPNsense's WAN (🟡, straight from the Fritz!Box, never through the switch), and 2.5G #2 is a dedicated management NIC (🔴, untagged VLAN 30) so this machine can be reached without going through the router VM it hosts. SFP+ 2 stays free. No other mini PC in this price class brings native 10G. Full scheme: [`cabling.md`](../../networking/cabling.md).
